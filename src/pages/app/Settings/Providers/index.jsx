...
const Providers = () => {
  ...
  const fetchProviders = useCallback((silent = false) => {
    ...
    get('/providers')
      ...
      .catch((err) => !silent && setError(err?.message ?? strings('error.failedLoadProviders')))
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="flex items-center gap-2 text-2xl font-semibold text-slate-900 dark:text-white">
          <i className="fa-solid fa-credit-card text-slate-600 dark:text-slate-400" aria-hidden />
      ...
      <SlidePanel
        ...
        title={id === 'new' ? strings('form.provider.newTitle') : strings('form.provider.editTitle')}
        aria-label={id === 'new' ? strings('form.provider.newTitle') : strings('form.provider.editTitle')}
      >
        {id && (
          <ProviderPanel
            id={id}
