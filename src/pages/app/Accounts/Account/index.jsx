...
const AccountPanel = ({ id, onClose }) => {
  ...
  useEffect(() => {
    ...
    get(`/accounts/${id}`)
      ...
      .catch((err) => setError(err?.message ?? strings('error.failedLoadAccounts')))
  ...
  const onSave = async (formData) => {
    ...
    try {
      ...
      setData((prev) => (prev ? { ...prev, ...(res.data ?? payload) } : res.data ?? payload))
    } catch (err) {
  ...
  return (
    <div className="flex h-full flex-col">
      <header className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 px-6 py-4">
        <h2 className="text-lg font-semibold text-slate-900">{strings('page.accounts.details')}</h2>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          ...
          <form id="account-form" onSubmit={handleSubmit(onSave)} className="space-y-4">
            {error && (
