	...
	const onSave = async (formData) => {
		...
		try {
			const cleanDomains = (formData.domains ?? [])
				...
				.map((d) => ({ hostname: d.hostname.trim(), service: d.service || "dashboard" }));
			const payload = {
			...
			if (isNew) {
				...
				setData((prev) => (prev ? { ...prev, ...(res.data ?? payload) } : res.data));
				onSaved?.();
			}
		} catch (err) {
			...
			<div className="flex h-full flex-col">
				<header className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 px-6 py-4">
					<h2 className="text-lg font-semibold text-slate-900">
						{isNew ? strings("form.realm.newTitle") : strings("form.realm.editTitle")}
					</h2>
					<button
						type="button"
						onClick={onClose}
