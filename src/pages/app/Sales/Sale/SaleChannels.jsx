	...
	return (
		<div className="relative">
				...
				<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div>
						<h2 className="text-lg font-medium text-slate-900">
							{strings("form.channel.title")}
						</h2>
						<p className="mt-0.5 text-sm text-slate-500">
							{channels.length === 1
								? strings("form.channel.count", [channels.length])
			...
			<SlidePanel
				...
				aria-label={
					isAdding
						? strings("form.channel.addChannelPanel")
						: strings("form.channel.editChannelPanel")
				}
			>
				<ChannelPanel
								...
								<ResponsiveContainer width="100%" height="100%">
									...
									>
										...
										<Tooltip
											formatter={(value) => [
												value,
												strings("form.channel.count"),
											]}
											labelFormatter={(label) =>
												`${strings("form.channel.age")} ${label}`
											}
										/>
										<Bar
											dataKey="count"
...
const ChannelPanel = ({
	...
	const isBase =
		channel &&
		["base-sale", "base sale"].includes((channel?.name ?? "").toLowerCase());
							...
							<>
								...
								>
									{saving ? (
										...
										<>
											{isNew
												? strings("form.channel.createChannel")
