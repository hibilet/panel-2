...
const SaleCoupons = () => {
	...
	const fetchData = useCallback(() => {
		...
		return Promise.all([
			...
			.catch((err) =>
				setError(err?.message ?? strings("error.failedLoadCoupons")),
			)
			.finally(() => {
				setCouponsLoading(false);
				setChannelsLoading(false);
		...
		<div className="relative">
				...
				<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div>
						<h2 className="text-lg font-medium text-slate-900">
							{strings("form.coupon.title")}
						</h2>
						<p className="mt-0.5 text-sm text-slate-500">
							{coupons.length === 1
								? strings("form.coupon.count", [coupons.length])
			...
			<SlidePanel
				...
				aria-label={
					isAdding
						? strings("form.coupon.addCoupon")
						: strings("form.coupon.editCoupon")
				}
			>
				<CouponPanel
		...
		<div className="flex h-full flex-col">
			<header className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 px-6 py-4">
				<h3 className="text-lg font-semibold text-slate-900">
					{isNew
						? strings("form.coupon.newCoupon")
						: coupon?.code || strings("form.coupon.editCoupon")}
				</h3>
				<button
					type="button"
					onClick={onClose}
						...
						<div className="space-y-4">
							...
							<Select
								...
								options={[
									{
										value: "active",
										label: strings("form.coupon.statusActive"),
									},
									{
										value: "inactive",
										label: strings("form.coupon.statusInactive"),
									},
								]}
							/>
						</div>
					...
					<div className="flex flex-wrap gap-3">
						...
						>
							{saving ? (
								...
								<>
									{isNew
										? strings("form.coupon.createCoupon")
						...
						{isNew ? (
							...
							<button
								...
								title={
									hasUsed
										? strings("form.coupon.cannotDeleteUsed")
